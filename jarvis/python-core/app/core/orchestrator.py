...
class JarvisOrchestrator:
    """Cognitive coordinator with temporal, graph and episodic memory context."""
    ...
    def handle(self, command: str, event_sink=None) -> str:
        ...
        reasoning=self.reasoning.create_plan(ReasoningRequest(
            command=normalized,
            context={"working_memory":self.memory.recall("last_command"),"memory":context},
        ))
        action_plan=self.dag_planner.build(normalized)
        self.events.publish(Event("jarvis.plan.created",{
            "goal":action_plan.goal,"actions":len(action_plan.actions),
            "confidence":action_plan.confidence,"reasoning_steps":len(reasoning.steps),
        }),event_sink)
        ...
        for action in action_plan.actions:
            ...
            self.scheduler.submit(action.action_id,action.target,priority=action.priority)
        ...
        while True:
            ready=set(self.task_graph.ready_tasks(completed))
            if not ready:
                break
            ordered=[]
            while self.scheduler.size():
                scheduled=self.scheduler.next_task()
                if scheduled.task_id in ready:
                    ordered.append(scheduled.task_id)
            for task_id in ordered:
                ...
                if action.kind=="respond":
                    output={"status":"success","message":"Task graph completed.","goal":action_plan.goal,
                            "completed_actions":list(completed),"confidence":action_plan.confidence}
                elif action.kind=="delegate" and action.target=="security":
                    output={"status":"confirmation_required","message":"This action requires explicit confirmation."}
                    self.events.publish(Event("jarvis.security.blocked",{
                        "action_id":task_id,"reason":"explicit confirmation required",
                    }),event_sink)
                else:
                ...
                self.events.publish(Event("jarvis.action.completed",{
                    "action_id":task_id,"completed":len(completed),"total":len(action_plan.actions),
                }),event_sink)
